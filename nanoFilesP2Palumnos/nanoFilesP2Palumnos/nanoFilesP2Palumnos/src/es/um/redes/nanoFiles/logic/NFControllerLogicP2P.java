...
public class NFControllerLogicP2P {
	...
	protected NFControllerLogicP2P(FileDatabase fileDb) {
	
	NFControllerLogicP2P.fileDb = fileDb;
	}
	...
	protected boolean startFileServer() {
		...
		if (fileServer != null) {
			 ...
			 */
			try {
	            fileServer = new NFServer(fileDb);
	            Thread serverThread = new Thread(() -> {
	                try {
	                    fileServer.start();
	                } catch (IOException e) {
	                    System.err.println("Error running file server: " + e.getMessage());
	                }
	            });
	            serverThread.start();
	            Thread.sleep(500);
	...
	protected boolean downloadFileFromServers(InetSocketAddress[] serverAddressList, String targetFileNameSubstring,
		...
		try {
	        for (InetSocketAddress serverAddress : serverAddressList) {
	            ...
	            boolean chunkDownloaded = connector.downloadChunk(targetFileNameSubstring, localFileName);
	            if (chunkDownloaded) {
	                System.out.println("Downloaded chunk from " + serverAddress);
	                downloaded = true;
	            }
	        }
	        if (downloaded) {
	            String checksum = es.um.redes.nanoFiles.util.FileDigest.computeFileChecksumString(localFileName);
	            System.out.println("Download completed. Checksum: " + checksum);
	        } else {
	            System.err.println("Download failed from all servers.");
	        }
	    } catch (IOException e) {
	...
	protected boolean serving() {
	    boolean result = (fileServer != null);
		return result;
	...
	protected boolean uploadFileToServer(FileInfo matchingFile, String uploadToServer) {
		...
		try {
	        InetSocketAddress serverAddress = new InetSocketAddress(uploadToServer, NFServer.PORT);
	        NFConnector connector = new NFConnector(serverAddress);
	        result = connector.uploadFile(matchingFile);
	        if (result) {
	            System.out.println("File uploaded successfully to " + uploadToServer);
	        } else {
	            System.err.println("Failed to upload file to " + uploadToServer);
	        }
	    } catch (IOException e) {
